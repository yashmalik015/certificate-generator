...
async function run() {
  const sourceImage = '/Users/yashmalik/.gemini/antigravity-ide/brain/2d09883f-1947-4bd3-9515-56d10d8983e9/.user_uploaded/media_1789544139304.jpg';
  ...
  for (const d of destDirs) {
    if (fs.existsSync(d)) {
      const name = 'Arya Bhushan Samaj Seva Award';
      fs.writeFileSync(path.join(d, `${name}.png`), pngBuf);
      fs.writeFileSync(path.join(d, `${name}.pdf`), pdfBuf);
    }
  }
  console.log(`Updated Arya Bhushan template successfully. Resolution: ${baseImg.width}x${baseImg.height}`);
}
